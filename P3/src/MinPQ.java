...
public class MinPQ<E extends Comparable<E>> implements PriorityQueueADT<E>
{
    ...
    // declare a variable to keep track of the position of items
	...
	public boolean isEmpty() {
		// return true if the array contains nothing, else false
		return size() == 0;
	}
	...
	public void insert(E item) {
		...
		while(isDone)
		{
			...
			// if the parent item is small than the current child
			...
			// if the parent is bigger than child, then swap child and parent items
			else
			{
	...
	public E removeMax() throws EmptyQueueException {
		...
		while(!isDone)
		{
			...
			while(tempIndex * 2 < size())
			{
				...
				{
					// if the child item is bigger than parent item, then it executes
				...
				{
					// if child item is bigger than the next item
					if(child.compareTo(items[tempIndex * 2 + 1]) > 0)
					{
						...
						// swap child and parent item
						items[tempIndex] = child;
						
						// swapping
						items[(tempIndex - 1) / 2] = temp;
					}
					// if it is smaller then exit
					else
					{
